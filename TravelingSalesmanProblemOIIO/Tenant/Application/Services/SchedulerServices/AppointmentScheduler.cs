...
namespace AppointmentProj.Application.Services.SchedulerServices
{
    public class AppointmentScheduler : IAppointmentScheduler
    {
        ...
        public Dictionary<int, Appointment> ScheduleAppointments(List<AppointmentRequest> appointmentRequests, double[] costArray)
        {
            var disctionairy = new Dictionary<int, Appointment>();
            ...
            for(int appointmentIndex = 1; appointmentIndex < appointmentRequests.Count; appointmentIndex++)
            {
                ...
                disctionairy.Add(Req.Id, appointment);
